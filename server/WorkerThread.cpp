...
// Do the objects use different resources (model, textures)?  If so we may need to generate optimised and LOD meshes, and LOD and basis textures, for the new resources.
// Note that we don't consider the lightmap URL, as lightmaps are generated by the lightmapper bot, or the audio URL, as audio files don't have LOD levels.
static bool objectResourcesDiffer(const WorldObject& a, const WorldObject& b)
{
	if(a.model_url != b.model_url)
		return true;
	...
	{
		const WorldMaterial& mat_a = *a.materials[i];
		const WorldMaterial& mat_b = *b.materials[i];

		if((mat_a.colour_texture_url != mat_b.colour_texture_url) ||
			(mat_a.emission_texture_url != mat_b.emission_texture_url) ||
			(mat_a.roughness.texture_url != mat_b.roughness.texture_url) ||
			(mat_a.normal_map_url != mat_b.normal_map_url))
			return true;
	}
	...
	try
	{
		...
		// See if this is a resource that is used by an object.  If so, send a message to the MeshLodGenThread to generate LOD levels and KTX versions of it if applicable.
		{
			...
			for(size_t i=0; i<ob_uids.size(); ++i)
				server->enqueueMsgForLodGenThread(new CheckGenResourcesForObject(ob_uids[i]));
					...
					case Protocol::ObjectFullUpdate:
						{
							...
							else
							{
								...
								// If the object now uses different resources, send a message to the MeshLodGenThread to generate LOD levels, optimised meshes and basis textures
								// for them if needed.  The new resources may already be present on the server, in which case there is no upload to trigger the generation.
								if(resources_changed)
									server->enqueueMsgForLodGenThread(new CheckGenResourcesForObject(object_uid));
