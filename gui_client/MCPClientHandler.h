...
To test: 
In Powershell:
curl.exe -s http://localhost:8095/mcp -H "Content-Type: application/json" -d '{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/list\"}'
=====================================================================*/
class MCPClientRequestHandler : public web::RequestHandler
{
public:
	MCPClientRequestHandler(MainWindow* main_window, const std::string& username, const std::string& password);
...
{
public:
	MCPClientSharedRequestHandler(MainWindow* main_window_, const std::string& username_, const std::string& password_)
	:	main_window(main_window_), username(username_), password(password_) {}

	virtual Reference<web::RequestHandler> getOrMakeRequestHandler() override { return new MCPClientRequestHandler(main_window, username, password); }
